...
import { BookOpen, Plus, Search, Loader2, X, FileText, PlayCircle, Link as LinkIcon, Download, UserPlus, RotateCcw, User } from 'lucide-react';
...
const Library: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribeItems = subscribeToCollection('library', (data) => {
      setItems(data);
      setLoading(false);
    });
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ title: '', type: 'SERMON_NOTE', description: '', url: '', status: 'AVAILABLE' });
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleIssue = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDocument('library', selectedItem.id, {
        status: 'ISSUED',
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-1">Secure storage for sermons, teaching materials and records</p>
        </div>
        <button onClick={() => setShowModal(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-black py-3 px-6 rounded-2xl shadow-xl transition-all flex items-center gap-2">
          <Plus size={18} /> <span>Add Resource</span>
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {loading ? (
          <div className="col-span-full py-20 text-center text-slate-300 font-black"><Loader2 className="animate-spin inline mr-2" /> Indexing Library...</div>
        ) : items.length === 0 ? (
          <div className="col-span-full bg-white p-24 rounded-[3rem] border border-slate-100 text-center">
            <BookOpen size={64} className="text-slate-100 mx-auto mb-4" strokeWidth={1} />
            <p className="text-slate-400 font-black uppercase text-[10px]">Library Empty</p>
          </div>
        ) : items.map((item) => (
          <div key={item.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-indigo-100 transition-all flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start mb-6">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${item.type === 'SERMON_NOTE' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
                  {item.type === 'SERMON_NOTE' ? <FileText size={24} /> : <PlayCircle size={24} />}
                </div>
                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${item.status === 'AVAILABLE' || !item.status ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'}`}>
                  {item.status || 'AVAILABLE'}
                </span>
              </div>
              <h3 className="text-base font-black text-slate-900 mb-2 leading-tight">{item.title}</h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">{item.type.replace('_', ' ')}</p>
            ...
            <div className="space-y-2">
              <a href={item.url} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 bg-slate-900 text-white font-black py-3 rounded-xl hover:bg-slate-800 transition-all text-[10px] w-full">
              ...
              <div className="flex gap-2">
                {item.status === 'ISSUED' ? (
                  ...
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-indigo-50 text-indigo-600 font-black py-3 rounded-xl text-[10px] hover:bg-indigo-100 transition-all"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl shadow-2xl animate-in zoom-in duration-200">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Add Resource</h3>
            ...
            <form onSubmit={handleAdd} className="p-8 space-y-5">
              <div>
                ...
                <input required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:border-indigo-600 outline-none font-bold" placeholder="e.g. Sunday Sermon Notes - Faith in Chaos" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Category</label>
                  <select value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})} className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:border-indigo-600 outline-none font-bold">
                    <option value="SERMON_NOTE">Sermon Notes</option>
                    <option value="AUDIO">Audio Recording</option>
                    <option value="VIDEO">Video Link</option>
                    <option value="DOCUMENT">Policy/Church Record</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Link/URL</label>
                  <input required value={formData.url} onChange={(e) => setFormData({...formData, url: e.target.value})} className="w-full px-5 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:border-indigo-600 outline-none font-bold" placeholder="https://..." />
                </div>
              </div>
      ...
      {showIssueModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Issue Resource</h3>
            ...
            <form onSubmit={handleIssue} className="p-8 space-y-6">
              <div>
                <p className="text-xs font-bold text-slate-500 mb-4 uppercase tracking-widest">Issuing: <span className="text-indigo-600 font-black">{selectedItem?.title}</span></p>
      ...
      {showReturnModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in duration-200">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Return Resource</h3>
            ...
            <div className="p-8 space-y-6">
              <div className="text-center">
                ...
                <h4 className="text-lg font-black text-slate-900 leading-tight mb-4">{selectedItem?.title}</h4>
